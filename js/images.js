...
var data = Object.keys(images).length;

$(function () {
    for (var i = 0; i < data; i++) {
        var path = images[i].path
        var title = '"' + images[i].title + '"';
        $(".gallery").append('<li><img src= images/square/' + path + ' ' + 'alt=' + ' ' + title + ' ' + '></li>');
    }
    $(".gallery img")
        .on("mouseenter", function (e) {
            ...
            var updateLeft = x + "px";
            var y = e.pageY + 10;
            var updateRight = y + "px";
            var input = e.target;
            var source = $(input).attr("src");
            source = source.replace(/square/g, 'medium');
            var title = $(input).attr("alt");
            var country;
            var city;
            var date;
            for (var i = 0; i < data; i++) {
                if (images[i].title == title) {
                    country = images[i].country;
                    city = images[i].city;
                    date = images[i].taken;
                }
            }
            var pictureDetails = title + '</br>' + country + ', ' + city + ' (' + date + ')' + '</>';
            $('body').append('<div id="preview"><img src=' + source + ' ><br><p>' + pictureDetails + '</p></div>');
            $("div#preview").css({
                'left': updateLeft,
                'top': updateRight
            });
